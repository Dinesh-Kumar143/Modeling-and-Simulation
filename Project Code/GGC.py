...
def main():
    ...
    def customer():
        ...
        no_cus = math.ceil(float(In_dist_params[0]) * int(nu_hr.get()))
        return no_cus

    def on_submit():
        if not entry_In_Dis_Params.get() or not customer() or not entry_dist_params.get() or not entry_servers.get():
