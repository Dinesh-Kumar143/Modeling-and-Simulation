...
def main():
    ...
    tk.Label(input_frame, text="Number of hours:").grid(row=0, column=4, padx=5, pady=5)
    nu_hr = tk.Entry(input_frame)
    nu_hr.grid(row=0, column=5, padx=5, pady=5)
    ...
    def on_submit():
        if not entry_lmbda.get() or not customer() or not entry_dist_params.get() or not entry_servers.get():
