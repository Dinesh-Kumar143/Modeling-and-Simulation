...
def simulate_mmc(lmbda, mu, servers, arrivals, result_frame, chart_frame):
    ...
    def customer(env, pid, arrival_time):
        ...
        for idx in range(servers):
            # Server is available at arrival time
    ...
    def create_utilization_chart():
        ...
        ax.barh([f"Server {i + 1}" for i in range(servers)],
                utilizations, color="red", edgecolor="black")

        # Add values on top of bars
        for i, utilization in enumerate(utilizations):
            ax.text(utilization + 2, i,
                    f"{utilization:.2f}%", va="center", color="black")
    ...
    def create_turnaround_chart():
        ...
        ax.plot(range(1, len(turnaround_times) + 1), turnaround_times,
    ...
    def create_waiting_chart():
        ...
        ax.plot(range(1, len(waiting_times) + 1), waiting_times,
    ...
    def create_response_chart():
        ...
        ax.plot(range(1, len(response_times) + 1), response_times,
    ...
    def create_inter_arrival_chart():
        fig, ax = plt.subplots(figsize=(10, 6))
        ax.plot(range(1, len(inter_arrival_times) + 1),
                inter_arrival_times, marker='o', linestyle='-', color='b')
    ...
    def create_dotted_plot():
        fig, ax = plt.subplots(figsize=(10, 6))
        ax.plot(range(1, len(arrival_times) + 1), arrival_times,
                marker='o', linestyle=':', color='g')
    ...
    for server_idx in range(servers):
        ...
        # Changed to side=tk.TOP
        gantt_button.pack(side=tk.TOP, padx=5, pady=5)

    # Create Utilization button and pack in the utilization_frame
    utilization_button = tk.Button(
        utilization_frame, text="Show Server Utilization", command=show_utilization_chart)
    # Changed to side=tk.TOP
    utilization_button.pack(side=tk.TOP, padx=5, pady=5)

    # Create all time-related buttons and pack in the time_frame
    turnaround_button = tk.Button(
        time_frame, text="Show Turnaround Times", command=show_turnaround_chart)
    # Changed to side=tk.TOP
    turnaround_button.pack(side=tk.TOP, padx=5, pady=5)

    waiting_button = tk.Button(
        time_frame, text="Show Waiting Times", command=show_waiting_chart)
    waiting_button.pack(side=tk.TOP, padx=5, pady=5)  # Changed to side=tk.TOP

    response_button = tk.Button(
        time_frame, text="Show Response Times", command=show_response_chart)
    response_button.pack(side=tk.TOP, padx=5, pady=5)  # Changed to side=tk.TOP

    inter_arrival_button = tk.Button(
        time_frame, text="Show Inter-Arrival Times", command=show_inter_arrival_chart)
    # Changed to side=tk.TOP
    inter_arrival_button.pack(side=tk.TOP, padx=5, pady=5)

    dotted_plot_button = tk.Button(
        time_frame, text="Show Customer Arrival Times", command=show_dotted_plot)
    # Changed to side=tk.TOP
    dotted_plot_button.pack(side=tk.TOP, padx=5, pady=5)
...
def main():
    ...
    tk.Label(input_frame, text="Number of hours:").grid(
        row=0, column=6, padx=5, pady=5)
    nu_hr = tk.Entry(input_frame)
    nu_hr.grid(row=0, column=7, padx=5, pady=5)
    ...
    def customer():
        no_cus = math.ceil(float(entry_lmbda.get()) * int(nu_hr.get()))
        return no_cus

    def on_submit():

        # Check if any of the input fields are empty
        if not entry_lmbda.get() or not entry_mu.get() or not entry_servers.get() or not customer():
    ...
    tk.Button(input_frame, text="Start Simulation", command=on_submit).grid(
